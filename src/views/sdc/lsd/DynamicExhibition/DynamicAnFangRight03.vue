<template>
  <div style="width:100%;height:100%;display: flex;align-items: flex-end;">
    <div ref="dynamicAnFangRight03" class="emgCountChart"></div>
  </div>
</template>

<script>
import echarts from 'echarts/lib/echarts'
import { getSecurityQuantityTrendStatistics } from "@/api/sdc/lsd";
...
export default {
  ...
  methods: {
    async queryPageData() {
      //获取初始区域查询条件数据
      getSecurityQuantityTrendStatistics({ siteId: this.address }).then((res) => {
        if (res.data.code === 0) {
          console.log(res);
          this.XName = res.data.data.date
          this.data1 = res.data.data.value
          this.coordsList = []
          for (let index = 0; index < res.data.data.date.length; index++) {
              const el = res.data.data.date[index];
              let arr = []
              arr.push(el)
              arr.push(res.data.data.value[index])
              this.coordsList.push(arr)
          }
          this.linesData.push({
    ...
    init() {
        let that = this
        //数据
        // let XName = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]
        // let data1 = [67, 97, 51, 338, 32, 11, 1]
        var img = [
        ...
        let dynamicAnFangRight03 = this.$echarts.init(this.$refs.dynamicAnFangRight03);
        dynamicAnFangRight03.setOption(option);
    }
  },
