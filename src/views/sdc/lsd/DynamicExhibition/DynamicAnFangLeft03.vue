<template>
  <div style="width:100%;height:100%;display: flex;align-items: center;">
    <div ref="dynamicAnFangLeft02" class="emgCountChart"></div>
  </div>
</template>
...
import { getDeviceOperationStatus } from "@/api/sdc/lsd";
export default {
  name: "EmgCount",
  data() {
    return {
  ...
  methods: {
    async queryPageData() {
      //获取初始区域查询条件数据
      getDeviceOperationStatus({ siteId: this.address }).then((res) => {
        if (res.data.code === 0) {
          console.log(res);
          this.trafficWay = []
          for (let index = 0; index < res.data.data.paramname.length; index++) {
              ...
              this.trafficWay.push({
                  name: el,
                  value: res.data.data.value[index]
              })
          }
          this.$nextTick(() => {
            this.init();
    ...
    init() {
        ...
        let dynamicAnFangLeft02 = this.$echarts.init(this.$refs.dynamicAnFangLeft02);
        dynamicAnFangLeft02.setOption(option);
    }
  }
};
</script>
