  ...
  <div>
    <basic-container>
      <el-row :gutter="15">
        ...
        <el-col :span="4">
          <span>支付方式：</span>
          <el-select v-model="paymentMethods" style="width:60%">
            <el-option
...
import {  getSdcParkuselogPayTable } from "@/api/sdc/parkMonitor";
...
export default {
  ...
  methods: {
    ...
    async queryInfo() {
      let tableData = await getSdcParkuselogPayTable(this.queryCondition)
    ...
    setQueryCondition() {
      ...
      if(this.appearanceDate) {
        console.log(moment(this.appearanceDate[0]).format('YYYY-MM-DD HH:mm:ss'),'moment(this.appearanceDate[0]).format');
