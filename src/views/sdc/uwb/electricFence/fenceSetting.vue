...
<div class="erweiClass" style="height:100%;">
  ...
  <!-- 二维三维显示位置 -->
  <div style="height:calc(100% - 43px);background-color:#f1f9fb;padding:10px;">
...
export default Vue.extend({
  ...
  methods: {
    initPolygons(){
      this.fencePinitList.forEach(item=>{
    ...
    updateObject(eventData, target) {
      console.log('测试',target.target.tempData)
      let self = this
    ...
    refreshList(){
      ...
      this.$http({
        ...
        params: {
          ...
          // 'fenceType':this.typeData
        }
      }).then(({data}) => {
        console.log('fencePinitList',data.page.list)
        if (data && data.success) {
          this.fencePinitList = data.page.list
          this.initPolygons()
        }
      })
    },
    ...
    polygonEdit(flag){
      // let count = this.fencePinitList.count
    ...
    fabricEvent() {
      this.fabricObj.on({
        ...
        "mouse:dblclick": e => {
          ...
          this.fenceList.push(this.roof)// 用于初始化删除

          console.info(xList);
          console.info(yList)

          //绘制完自动取消绘制
          this.$refs.fenceSettingForm.initData(this.valueMap,xList,yList,this.roofPoints)
