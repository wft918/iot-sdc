  ...
  <div>
    <basic-container>
        ...
        <el-col :span="24" style="margin-top:20px">
            ...
            >
              ...
              <el-table-column 
                ...
                align="center"
              ></el-table-column>
              <!-- <el-table-column  label="目标列表" prop="contactsName" align="center">
              </el-table-column > -->
              <el-table-column  
                label="目标列表" 
        ...
        >
          ...
          <el-form-item label="动作" prop="action">
            <el-radio-group v-model="inputForm.action">
              <el-radio :label="'11'">播放</el-radio>
              <el-radio :label="'12'">关闭</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="媒体资源" prop="param">
            <el-select v-model="inputForm.param" style="width:55%">
              <el-option
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="saveTarget">确 定</el-button>
...
import { getSdcLightTaskTable,sdcLightTaskAction,sdcLightTaskTargetAction,getSdcLightFileTable,deleteSdcLightTask} from "@/api/sdc/lpmanage";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        param: [{ required: true, message: "请选择媒体资源", trigger: "change" }],
        action: [
  ...
  methods: {
    ...
    async getMediaInfo() {
      this.mediaList = []
      let tableData = await getSdcLightFileTable({
        current: 1,
        size: 200,
        deviceTypeCode:"light_broadcast"
      })
    ...
    async tableAction(row,flag) {
      ...
      if(flag === 3) {
        ...
        this.dialogTitle = '编辑广播策略'
        
        this.inputForm = clone(row)
        this.inputForm.actionType = 2
        this.dueDate.push(this.inputForm.beginDate)
        this.dueDate.push(this.inputForm.endDate)
        // this.setlampAction(this.inputForm.action)
        this.inputForm.triggerDays = this.inputForm.triggerDays.split(',')
        this.dialogFormVisible = true
      } else if( flag === 4) {
        this.policyForm.taskId = row.id
        let tableData = await getSdcLightBroadcastTable(this.queryPolicyCondition);
        const promise = tableData.data.data.records.map( el => {
          ...
          for(let device of this.policyData) {
            if(row.expand.targetIds) {
              row.expand.targetIds.forEach(item => {
                if (device.id === item) {
                  this.$nextTick(() => {
    ...
    doChange(val) {
      switch (val) {
        case 11:
          this.inputForm.action = '11'
          break;
        case 12:
          this.inputForm.action = '12'
          break;
      }
    },
    changeSite() {
    ...
    saveTarget() {
      this.policyForm.targetIds = this.selectedForm.map(item => item.id)
