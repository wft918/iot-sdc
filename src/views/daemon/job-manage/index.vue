    ...
    methods: {
      ...
      save(row, done, loading) {
        isValidTaskName({
          ...
          if (result !== 0) {
            this.$message.error('任务名称与任务组重复，请确认后重新添加')
          } else {
            addObj(row).then(() => {
      ...
      handleDelete(row) {
        ...
        if (jobStatus === '1' || jobStatus === '3') {
          ...
          this.$message.error('运行中定时任务不可删除，请先暂停后操作')
        }
      },
      handleJobLog(row) {
